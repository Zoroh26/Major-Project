...
def lifespan_factory(
    ...
    async def lifespan(app: FastAPI) -> AsyncGenerator:
        ...
        try:
            # --- safely handle optional Redis setup ---
            if isinstance(settings, RedisCacheSettings) and getattr(settings, "CRUD_ADMIN_REDIS_ENABLED", False):
                try:
                    await create_redis_cache_pool()
                    print("Redis cache initialized.")
                except Exception as e:
                    print(
                        f"Skipping Redis cache init (connection failed): {e}")

            if isinstance(settings, RedisQueueSettings) and getattr(settings, "CRUD_ADMIN_REDIS_ENABLED", False):
                try:
                    await create_redis_queue_pool()
                    print("Redis queue initialized.")
                except Exception as e:
                    print(
                        f"Skipping Redis queue init (connection failed): {e}")
