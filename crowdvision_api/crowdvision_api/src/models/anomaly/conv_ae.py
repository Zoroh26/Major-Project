    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if self.shrink_thres > 0:
                ...
                # Without shrinkage, cosine similarities can be negative,
                # making manual sum-normalisation unstable.  Use softmax
                # to get a proper probability distribution over slots.
...
class ConvAE(nn.Module):
    ...
    Trained only on normal frames.  A tight bottleneck forces the network
    to learn a compact representation of *normal* appearance; anomalous
    inputs should incur higher reconstruction error.
    ...
    def __init__(self, in_channels: int = 1, base_ch: int = 32,
                 mem_slots: int = 500, shrink_thres: float = 0.005):
        super().__init__()
        c = base_ch
        ...
        # Bottleneck – squeeze channels to create a real information
        # bottleneck (c*8 → c*2, i.e. 256 → 64 channels at 8×12 spatial
        # = 6 144-dim latent).
        self.bottleneck_enc = nn.Sequential(
            nn.Conv2d(c * 8, c * 2, 1, bias=False),   # 1×1 squeeze
            nn.BatchNorm2d(c * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(0.5),
        )
        ...
        self.memory = MemoryModule(
            num_slots=mem_slots, slot_dim=c * 2,
            shrink_thres=shrink_thres,
        )

        self.bottleneck_dec = nn.Sequential(
            nn.Conv2d(c * 2, c * 8, 1, bias=False),   # 1×1 expand
            nn.BatchNorm2d(c * 8),
            nn.ReLU(inplace=True),
        )

        self.code_ch = c * 2  # effective bottleneck channels
