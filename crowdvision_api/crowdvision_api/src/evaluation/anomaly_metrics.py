...
def evaluate_anomaly_detection(model, train_loader, test_loader,
    ...
    for batch in test_loader:
        ...
        if is_prediction and is_clip:
            ...
            # reconstruction_error handles past/future split internally
            frames_eval = frames
            B = frames.shape[0]
        ...
        # Average per-frame errors back to per-clip if we flattened
        if not is_prediction and is_clip and not hasattr(model, 'temp_enc'):
            err = err.reshape(B, T).mean(axis=1)
